...
static void send_file(int client_socket) {
    auto t0 = std::chrono::steady_clock::now();

    // ---- receive filename_length (uint32_t network order) ----
    ...
    // ---- send file data + compute SHA256 while sending ----
    SHA256_CTX ctx;
    SHA256_Init(&ctx);
...
int main() {
    ...
    while (true) {
        ...
        if (close(client_socket) < 0) {
            die("close(client_socket) failed");
        }
    }
